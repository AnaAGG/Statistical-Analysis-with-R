...
points(occs.sp, pch=21, bg=jack$occ.grp)  # note that colors are repeated here

#Running ENMeval
eval1 <- ENMevaluate(presences, envs, bg, method='jackknife', RMvalues=c(1,2), fc=c('Q'), kfolds = 10, overlap = TRUE, algorithm='maxent.jar')
...
eval1 <- ENMevaluate(occ= presences, env = envs, 
                     ...
                     fc = c("L", "Q"), #El ajuste de nuestras regresiones, L = Linear, Q = cuadratico, H = Hinge, T = Threshol<d 
...
me <- maxent(envs, presences,  path="C:/Preba")

eval1 <- maxent(occ= presences, env = envs, 
                     bg.coords = bg, # matriz o data. frame de dos columnas de la long y lat de los puntos de background
                     categoricals = NULL, method = "jackknife", #categoricals = indicar si hay alguna variable categorica
                     occ.grp = NULL, bg.grp = NULL, 
                     fc = c("L", "Q"), #El ajuste de nuestras regresiones, L = Linear, Q = cuadratico, H = Hinge, T = Threshol<d 
...
args = c('responsecurves=true', 'writeplotdata=true'), 
eval.plot(eval1@results)

#We can also plot the permutation importance or percent contribution.
...
eval1@results
eval1@models #se abre el explorador y se ven los resultados como los vemos en MaxEnt
eval1@predictions
eval1@overlap

response(eval1@models[[1]])
